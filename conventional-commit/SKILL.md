...
7. **Re-attach submodule HEADs**: If the repo has submodules (`.gitmodules` exists), re-attach any detached submodules to their tracking branch: `git submodule foreach 'git checkout main 2>/dev/null || git checkout master 2>/dev/null || true'`. Do NOT use `git submodule update` after a local merge — it detaches HEAD from the branch, making VS Code show commit hashes instead of branch names.
...
### 5.3 Submodule Commit Hygiene

When committing inside a submodule (not the parent repo):

1. **Ensure on branch**: Before any work, check if the submodule is in detached HEAD state (`git symbolic-ref --short HEAD 2>/dev/null`). If detached, run `git checkout main` (or `master`) first to re-attach.
2. **Branch/merge/delete cycle**: Follow the same §5.2 flow (create branch → commit → merge → delete branch). After the merge, the submodule stays on `main`.
3. **Never leave detached**: After all operations, verify the submodule is on a named branch, not a commit hash. VS Code's Source Control panel shows commit hashes for detached HEADs, which is confusing.
